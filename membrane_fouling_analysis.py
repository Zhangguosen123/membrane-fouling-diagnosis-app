...
import matplotlib.font_manager as mpl_font  # 导入字体管理模块
import pandas as pd
import os
...
# ==============================================================================
# <<-- 核心：注册根目录下的SimHei.ttf字体（适配你的仓库结构）
# ==============================================================================
# 字体文件路径（仓库根目录/SimHei.ttf）
font_path = os.path.join(os.path.dirname(__file__), 'SimHei.ttf')
# 临时注册字体（关键：让Matplotlib识别SimHei字体）
if os.path.exists(font_path):
    mpl_font.fontManager.addfont(font_path)
    # 配置全局字体，正常显示中文和负号
    plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定SimHei为默认中文字体
    plt.rcParams['axes.unicode_minus'] = False    # 解决负号显示为方块的问题
else:
    st.warning(f"⚠️ 未找到SimHei.ttf字体文件（路径：{font_path}），中文可能显示异常！")
    # 降级使用云端预装字体（备用）
...
def fit_model(t, J_obs, J0):
    """模型拟合主函数：调用遗传算法求解参数"""
    if len(t) < 5:
        return np.array([0.1,0.1,0.1,0.1,0.5,0.5])  # 数据不足时返回默认参数
...
def calculate_mechanism_contribution(params, t, J0):
    ...
    for si in [s1, s2, s3, s4]:
        val = -np.trapz(si, t)  # 积分计算贡献度
        Di.append(max(val, 0.0))
    Dsum = sum(Di) + EPS
    eta = np.array([d / Dsum for d in Di])  # 归一化得到占比
    return eta  # [标准污染, 完全污染, 中间污染, 滤饼污染]
...
def recommend_cleaning_strategy(eta, stage="full"):
    ...
    if stage == "partial":  # 100%-70%阶段
        ...
        elif dominant_idx == 0 or dominant_idx == 1:  # 标准/完全污染
            ...
            if res["success"]:
                with st.expander(f"📄 {res['filename']} 详细报告", expanded=False):
                    ...
                    with col1:
                        ...
                        ax1.pie(filtered_sizes_full, labels=filtered_labels_full, autopct='%1.1f%%', 
                    ...
                    with col2:
                        ...
                        ax2.pie(filtered_sizes_partial, labels=filtered_labels_partial, autopct='%1.1f%%', 
                                startangle=90, colors=['#ff9999','#66b3ff','#99ff99','#ffcc99'])
                        ax2.axis('equal')
                        st.pyplot(fig2)
